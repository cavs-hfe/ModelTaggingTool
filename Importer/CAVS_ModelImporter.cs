...
namespace ModelViewer.Importer
{
    ...
    public class CAVS_ModelImporter
    {
        ...
        public Model3DGroup Load(string path, Dispatcher dispatcher = null, bool freeze = false)
        {
            ...
            switch (ext)
            {
                ...
                case ".obj":
                    {
                        var r = new CAVS_ObjReader(dispatcher) { DefaultMaterial = this.DefaultMaterial, Freeze = freeze };
                        model = r.Read(path);
                        break;
                    }
                case ".objz":
